...
def _is_valid_length_and_checksum(msg: str) -> tuple[bool, str]:
    ...
    try:
        ...
        if (calculate_crc16_checksum(0, msg) != 0:
...
def vn_decode(msg: str) -> dict[str, str]:
    ...
    elke27_version = f"{int(msg[4:6], 16)}.{int(msg[6:8], 16)}.{int(msg[8:10], 16)}"
    xep_version = f"{int(msg[10:12], 16)}.{int(msg[12:14], 16)}.{int(msg[14:16], 16)}"
    return {"elke27_version": elke27_version, "xep_version": xep_version}
