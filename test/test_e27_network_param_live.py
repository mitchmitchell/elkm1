...
async def test_live_network_param_getters() -> None:
    ...
    evt = _wait_for_event(elk, NetworkSsidResultsUpdated.KIND, timeout_s)
    if evt is None:
        await elk.close()
        pytest.fail("Timed out waiting for NetworkSsidResultsUpdated event.")
    print(f"SSID results count: {evt.count}")

    seq = elk.request(("network", "get_rssi"))
    print(f"Sent network.get_rssi seq={seq}")
    evt = _wait_for_event(elk, NetworkRssiUpdated.KIND, timeout_s)
    if evt is None:
        await elk.close()
        pytest.fail("Timed out waiting for NetworkRssiUpdated event.")
    print(f"RSSI: {evt.rssi}")
...
def _wait_for_event(elk: Elk, kind: str, timeout_s: float):
    ...
    while time.monotonic() < deadline:
        ...
        for evt in elk.drain_events():
            ...
            if evt.kind == DispatchRoutingError.KIND and evt.code == "domain_multi":
                pytest.skip(f"{kind} blocked by authorization: {evt.message}")
            if evt.kind == kind:
                return evt
    return None
