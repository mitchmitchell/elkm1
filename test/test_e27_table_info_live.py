...
async def test_live_table_info_calls() -> None:
    ...
    seq = elk.request(("area", "get_table_info"))
    print(f"Sent area.get_table_info seq={seq}")

    evt = _wait_for_event(elk, AreaTableInfoUpdated.KIND, timeout_s)
    if evt is None:
        await elk.close()
        pytest.fail("Timed out waiting for AreaTableInfoUpdated event.")
    print(f"Area table: elements={evt.table_elements} increment={evt.increment_size}")

    seq = elk.request(("zone", "get_table_info"))
    print(f"Sent zone.get_table_info seq={seq}")

    evt = _wait_for_event(elk, ZoneTableInfoUpdated.KIND, timeout_s)
    if evt is None:
        await elk.close()
        pytest.fail("Timed out waiting for ZoneTableInfoUpdated event.")
    print(f"Zone table: elements={evt.table_elements} increment={evt.increment_size}")

    seq = elk.request(("output", "get_table_info"))
    print(f"Sent output.get_table_info seq={seq}")

    evt = _wait_for_event(elk, OutputTableInfoUpdated.KIND, timeout_s)
    if evt is None:
        await elk.close()
        pytest.fail("Timed out waiting for OutputTableInfoUpdated event.")
    print(f"Output table: elements={evt.table_elements} increment={evt.increment_size}")

    seq = elk.request(("tstat", "get_table_info"))
    print(f"Sent tstat.get_table_info seq={seq}")

    evt = _wait_for_event(elk, TstatTableInfoUpdated.KIND, timeout_s)
    if evt is None:
        await elk.close()
        pytest.fail("Timed out waiting for TstatTableInfoUpdated event.")
    print(f"Tstat table: elements={evt.table_elements} increment={evt.increment_size}")

    await elk.close()


def _wait_for_event(elk: Elk, kind: str, timeout_s: float):
