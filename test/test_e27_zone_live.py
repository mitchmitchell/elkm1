...
from elke27_lib.events import ApiError, ZoneConfiguredUpdated, ZonesStatusBulkUpdated
...
async def test_live_zone_get_configured_and_status() -> None:
    ...
    seq = elk.request(("zone", "get_configured"))
    print(f"Sent zone.get_configured seq={seq}")
    ...
    while time.monotonic() < deadline:
        ...
        for evt in elk.drain_events():
            if evt.kind == ApiError.KIND:
                await elk.close()
                pytest.skip(f"zone.get_configured not supported: error_code={evt.error_code}")
            if evt.kind == ZoneConfiguredUpdated.KIND:
                configured_seen = True
                print(f"Configured zones count: {len(evt.configured_ids)}")
                break
        if configured_seen:
            break
    ...
    if not configured_seen:
        await elk.close()
        pytest.fail("Timed out waiting for ZoneConfiguredUpdated event.")

    seq = elk.request(("zone", "get_all_zones_status"))
    print(f"Sent zone.get_all_zones_status seq={seq}")
    ...
    while time.monotonic() < deadline:
        ...
        for evt in elk.drain_events():
            if evt.kind == ApiError.KIND:
                await elk.close()
                pytest.skip(f"zone.get_all_zones_status not supported: error_code={evt.error_code}")
            if evt.kind == ZonesStatusBulkUpdated.KIND:
                status_seen = True
                print(f"Bulk zone status updated count: {evt.updated_count}")
                break
        if status_seen:
            break
    ...
    await elk.close()
    if not status_seen:
        pytest.fail("Timed out waiting for ZonesStatusBulkUpdated event.")
